...
- (nullable instancetype)initWithJSONData:(NSData *)serviceDiscoveryJSONData
  ...
  if (issuer && [issuer containsString:@"{tenantid}"]) {
    // The Azure AD discovery document's "issuer" value contains the special placeholder "{tenantid}". '{' and '}' are
    // invalid characters in URLs and have to be URL encoded before the issuer URL can be parsed by NSURL.
